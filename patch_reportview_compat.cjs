...
const regex = /\{\/\* Compatibility Results \*\/\}[\s\S]*?(?=<\/div>\s*<\/div>\s*<\/motion\.div>\s*\)\s*\}\s*\{\/\* End of tabs \*\/\})/g;
...
fs.writeFileSync('src/components/ReportView.tsx', newCode);
console.log('ReportView updated');
